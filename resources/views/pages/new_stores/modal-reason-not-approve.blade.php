    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body px-sm-2 mx-50 pb-3 pt-2">
                ...
                <input type="hidden" id="form-new-store-id" name="new-store-id" value="{{ $newStoreId }}">
    ...
    <script defer>
        $(document).ready(function () {
            $('#button-submit-reason-not-approve').click(function () {
                let newStoreId = $('#form-new-store-id').val(),
                    reason = $('#form-reason').val(),
                    url_not_approve = "{{ route('admin.not-approve-new-stores') }}",
                    url_new_stores = "{{ route('admin.new-stores.index') }}";
                    ...
                    ajax(url_not_approve, 'POST', {
                        ...
                        if (response.htmlString === "success") {
                            Swal.fire({
                                ...
                                window.location.href = url_new_stores;
                            })
                        } else {
                            Swal.fire({
                                icon: 'error',
