...
{{--Chiết khấu tổng số lượng--}}

<div class="wrap-repeater-promotion mt-1 inner-repeater template-type @if($typeDefault != 'type' . PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY) d-none @endif"
     data-template-type="type{{ PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY }}">
    <div data-repeater-list="type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}" style="position: relative" class="border-secondary rounded">
        @if(count($infoTypeCondition['type' . PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY] ?? []))
{{--Edit screen--}}
            @foreach($infoTypeCondition['type' . PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY] ?? [] as $condition)
            ...
            <div data-repeater-item class="p-1 border-bottom">
                <!-- innner repeater -->
                <div class="p-1 condition-type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}">
                    <div class="row">
                        ...
                        <label class="col-xl-10 col-md-10 col-sm-8 d-flex mb-1">
                            ...
                            <div class="w-100 include-product include-products">
                                <select class="has-select2 form-select select-product include-product" for-type="type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}"
                        ...
                        <label class="col-xl-10 col-md-10 col-sm-8 d-flex">
                            <select class="form-select me-1 select-type-exclude"
                                    ...
                                    for-type="type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}"
                                    name="excludeType"
                            >
                            ...
                            <div class="w-100 d-none exclude-product exclude-products">
                                <select class="has-select2 form-select select-product" for-type="type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}" multiple name="excludeProducts">
                                    <option value="all">Tất cả</option>
                                    @foreach($formOptions['products'] ?? [] as $key => $product)
                                        <option value="{{ $key }}">
                                            {{ $product }}
                            ...
                            <div class="w-100 d-none exclude-product exclude-groups">
                                <select class="has-select2 form-select select-product" for-type="type{{PromotionCondition::TYPE_DISCOUNT_BY_TOTAL_QTY}}" multiple name="excludeGroups">
                                    <option value="all">Tất cả</option>
                                    @foreach($formOptions['productGroups'] ?? [] as $key => $productGroup)
                                        <option value="{{ $key }}">
                                            {{ $productGroup }}
                        ...
                        <div class="deep-inner-repeater">
                            <table class="table table-striped table-bordered mb-1 sub-condition" data-repeater-list="gifts">
                                ...
                                @foreach($condition['conditions'] ?? [] as $conditionGift)
                                    ...
                                    <td>
                                        <input type="number" class="form-control" name="minQty" value="{{ $conditionGift['minQty'] ?? null }}" min="1"/>
                                    </td>
                                    <td>
                        ...
                        <div class="deep-inner-repeater">
                            <table class="table table-striped table-bordered mb-1 sub-condition" data-repeater-list="gifts">
                                ...
                                <tbody>
                                    <tr data-repeater-item class="condition-row">
                                    <td>
                                        <input type="number" class="form-control" name="minQty" min="1"/>
                                    </td>
                                    <td>
