...
<div class="wrap-repeater-promotion mt-1 inner-repeater template-type @if($typeDefault != 'type' . PromotionCondition::TYPE_GIFT_BY_TOTAL_COST) d-none @endif"
     data-template-type="type{{ PromotionCondition::TYPE_GIFT_BY_TOTAL_COST }}">
    <div data-repeater-list="type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}" style="position: relative" class="border-secondary rounded">
        @if(count($infoTypeCondition['type' . PromotionCondition::TYPE_GIFT_BY_TOTAL_COST] ?? []))
{{--Edit screen--}}
            @foreach($infoTypeCondition['type' . PromotionCondition::TYPE_GIFT_BY_TOTAL_COST] ?? [] as $condition)
            ...
            <div data-repeater-item class="p-1 border-bottom">
                <!-- innner repeater -->
                <div class="p-1 condition-type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}">
                    <div class="row">
                        ...
                        <label class="col-xl-10 col-md-10 col-sm-8 d-flex mb-1">
                            ...
                            <div class="w-100 include-product include-products">
                                <select class="has-select2 form-select select-product include-product" for-type="type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}"
                        ...
                        <label class="col-xl-10 col-md-10 col-sm-8 d-flex">
                            <select class="form-select me-1 select-type-exclude"
                                    ...
                                    for-type="type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}"
                                    name="excludeType"
                            >
                            ...
                            <div class="w-100 d-none exclude-product exclude-products">
                                <select class="has-select2 form-select select-product" for-type="type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}" multiple name="excludeProducts">
                                    <option value="all">Tất cả</option>
                                    @foreach($formOptions['products'] ?? [] as $key => $product)
                            ...
                            <div class="w-100 d-none exclude-product exclude-groups">
                                <select class="has-select2 form-select select-product" for-type="type{{PromotionCondition::TYPE_GIFT_BY_TOTAL_COST}}" multiple name="excludeGroups">
                                    <option value="all">Tất cả</option>
                                    @foreach($formOptions['productGroups'] ?? [] as $key => $productGroup)
                        ...
                        <div class="deep-inner-repeater">
                            <table class="table table-striped table-bordered mb-1 sub-condition" data-repeater-list="gifts">
                                ...
                                <tr data-repeater-item>
                                    ...
                                    <td>
                                        <input type="number" class="form-control min-type" name="minType" min="1"
                                           max="{{ count($formOptions['products'] ?? []) }}" value="{{ $conditionGift['minType'] ?? null }}"/>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <input type="number" class="form-control" name="giftQty" value="{{ $conditionGift['giftQty'] ?? null }}" min="1"/>
                                    </td>
                                    <td>
                                        <center>
                ...
                <div class="row">
                    ...
                    <div class="deep-inner-repeater">
                        <table class="table table-striped table-bordered mb-1 sub-condition" data-repeater-list="gifts">
                            ...
                            <tbody>
                            <tr data-repeater-item>
                                <td><input type="number" class="form-control" name="minCost" value="" min="1"/></td>
                                <td>
                                    <input type="number" class="form-control min-type" name="minType" max="{{ count($formOptions['products'] ?? []) }}" value="" min="1"/>
                                </td>
                                <td>
                                    <select name="gift" class="has-select2 form-select select-gift">
                                        <option value="">Chọn quà</option>
