...
@section('content')
    <section class="app-user-list">
        <div class="row" id="table-striped">
            <div class="col-12">
                <div class="card">
                    <b style="padding: 25px 25px 0px">
                        @if(isset($pharmacy_info))
                            NT: {{$pharmacy_info->store->name}} <br>
                            Địa bàn:{{$pharmacy_info->store->organization->name}} <br>
                            Địa chỉ: {{$pharmacy_info->store->address}}
                        @else
                            {{$title_table}}
                        @endif
                    </b>
