@if(!$empty)
    <div class="card-body d-flex pt-0 table-show-option">
    ...
    <div class="table-responsive">
        <table id="{{ $nameTable }}" class="table table-striped table-bordered">
                ...
                <tr>
                    @foreach($headers as $key => $attribute)
                        @if(is_array($item->$key ?? null))
                            @if(isset($item->$key['hidden'])) @continue @endif
    ...
    @if($isPagination)
        <div class="row">
            <div class="col-sm-12 mt-1 d-sm-flex justify-content-center">
                {{ $collections->appends(request()->query())->onEachSide(1)->links('snippets.custom-pagination-mobile') }}
            </div>
        </div>
    @endif
@else
