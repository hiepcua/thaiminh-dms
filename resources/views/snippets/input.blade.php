@switch($type)
    ...
    @case("select2")
        ...
        class='form-control has-select2 form-select'
                name='{{ $name }}'
        >
            @foreach($options as $key => $option)
                <option value="{{ $key }}" class="{{ ($other_options['option_class'] ?? '') }}"
                        @if($key == $defaultValue) selected @endif>{{ $option }}</option>
            @endforeach
        </select>
        @break
    @case("multipleSelect2")
        ...
        @break
    @case("checkbox")
        <label class="d-flex justify-content-center align-items-center h-100">
            <input class="form-check-input {{ $class }}" style="width: 18px !important;"
    ...
    @case("divisionPicker")
        ...
        <div style="min-width: {!! $widthDefault !!}">
            {!!
