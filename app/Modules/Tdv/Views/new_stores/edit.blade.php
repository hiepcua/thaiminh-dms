        ...
        <div class="card-body">
            <form id="form-add-edit-store" class="has-provinces" method="post" action="{{ $formOptions['action'] }}"
                            ...
                            <div class="col-md-6 col-12 mb-1">
                                ...
                                <select id="form-line_period" class="form-control" name="number_visit" required>
                                    @foreach(Store::LINE_PERIOD as $keyPeriod => $period)
                                        <option
                                            value="{{ $keyPeriod }}"
                                            {{ isset($default_values['number_visit']) && $default_values['number_visit'] == $keyPeriod ? 'selected':'' }}>{{ $period }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="text-center">
                    ...
                    <a href="{{ route('admin.tdv.new-stores.index') }}" class="btn btn-secondary me-1"><i
